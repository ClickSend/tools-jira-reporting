...
function getBasic(username, password) {
  var mix = username + ":" + password;
...
async function getFields( args ) {
  var path = "/rest/api/3/field"
  return await getJson( path, args );
}

async function getJson(path, args, headers) {
  var basic = getBasic(args.username, args.password);
...
async function getFieldId( name, args ) {

  if( fields === undefined ) {
    fields = await getFields( args );
  }
...
async function getUserFromUserName( user, args ) {
  var u = await getMatchingUsers(user, args);
...
async function getMatchingUsers(query, args) {
